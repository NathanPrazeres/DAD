...
public class DadkvsClient {
	...
	private boolean doCommit(int key1, int key1_version, int key2, int key2_version, int write_key, int write_value) {
		...
		if (commit_responses.size() >= responses_needed) {
			...
			if (result) {
				System.out
						.println("Committed key " + commit_request.getWritekey() + " with value " + commit_request.getWriteval());
			} else {
	...
	public void goInteractive() {
		...
		while (keep_going) {
			...
			switch (mainCommand) {
				...
				case "read":
					...
					if (parameter1 != null) {
						try {
							...
							if (kv_entry != null)
								System.out.println(
										"did read " + key + " with value " + kv_entry.getValue() + " and version " + kv_entry.getVersion());
							else
				...
				case "tx":
					System.out
							.println("tx reading key " + parameter1 + " and key " + parameter2 + " : writting key " + parameter3);
						...
						try {
							...
							if (write_key == 0)
								...
								if ((kv_entry1 != null) && (kv_entry2 != null))
									doCommit(read_key1, kv_entry1.getVersion(), read_key2, kv_entry2.getVersion(), write_key,
											write_value);
								else
	...
	public void main_loop(String[] args) throws Exception {
		...
		System.out
				.println("Client key_range = " + key_range + " sleep_range = " + sleep_range + " loop_size = " + loop_size);
